...
extract_lmm_stats <- function(model, trait_name, model_name, trait_var) {
  ...
  group_p <- if(length(group_idx) > 0) {
    2 * pt(abs(coefs[group_idx[1], "t value"]), df = df_res, lower.tail = FALSE)
  } else NA
  
  prev_idx <- grep("Prevalence_contrast", rownames(coefs))
  prev_p <- if(length(prev_idx) > 0) {
    2 * pt(abs(coefs[prev_idx[1], "t value"]), df = df_res, lower.tail = FALSE)
  } else NA
  
  inter_idx <- grep(":", rownames(coefs))
  inter_p <- if(length(inter_idx) > 0) {
    2 * pt(abs(coefs[inter_idx[1], "t value"]), df = df_res, lower.tail = FALSE)
  } else NA
  ...
  data.frame(
    ...
    Group_p = group_p, Prev_p = prev_p, Inter_p = inter_p,
...
lmm_summary %>%
  mutate(across(c(AIC, BIC), ~round(.x, 1)),
         across(c(Trait_Beta, Trait_SE), ~round(.x, 3)),
         across(c(Trait_t), ~round(.x, 2)),
         across(c(Trait_p, Group_p, Prev_p, Inter_p), ~round(.x, 4)),
         across(c(Subj_SD, Resid_SD), ~round(.x, 2)),
         Sig = case_when(Trait_p < 0.001 ~ "***",
                        Trait_p < 0.01 ~ "**",
                        Trait_p < 0.05 ~ "*",
                        TRUE ~ "")) %>%
  select(Trait, Model, AIC, BIC, Trait_Beta, Trait_SE, Trait_t, Trait_p, Sig,
         Group_p, Prev_p, Inter_p, Subj_SD, Resid_SD) %>%
  kable(col.names = c("Trait", "Model", "AIC", "BIC", "β", "SE", "t", "p", "",
                     "Group p", "Prev p", "Inter p", "Subj SD", "Resid SD"),
        caption = "Linear mixed models: Trait effects on distractor interference. β = ms change per 1 SD trait increase. Significance: *** p<0.001, ** p<0.01, * p<0.05") %>%
...
#| echo: false
#| fig-cap: "Trait-interference relationships by Group and Prevalence. Points = individual observations; lines = linear fits."
#| fig-width: 12
#| fig-height: 10
...
if (sum(!is.na(lmm_data$BDI)) > 2) {
  plot_list$BDI <- ggplot(lmm_data %>% filter(!is.na(BDI)),
                          aes(x = BDI, y = interference, color = Group, shape = Prevalence)) +
    geom_point(size = 2, alpha = 0.6) +
    geom_smooth(method = "lm", se = TRUE, alpha = 0.2, linewidth = 1) +
    facet_wrap(~Prevalence, ncol = 2) +
    scale_color_manual(values = c("TD" = "#E64B35FF", "ASD" = "#4DBBD5FF")) +
    scale_shape_manual(values = c("Low" = 16, "High" = 17)) +
    labs(x = "BDI Score", y = "Interference (ms)") +
    theme_classic(base_size = 11) +
...
if (sum(!is.na(lmm_data$STAI_Trait)) > 2) {
  plot_list$STAI <- ggplot(lmm_data %>% filter(!is.na(STAI_Trait)),
                           aes(x = STAI_Trait, y = interference, color = Group, shape = Prevalence)) +
    geom_point(size = 2, alpha = 0.6) +
    geom_smooth(method = "lm", se = TRUE, alpha = 0.2, linewidth = 1) +
    facet_wrap(~Prevalence, ncol = 2) +
    scale_color_manual(values = c("TD" = "#E64B35FF", "ASD" = "#4DBBD5FF")) +
    scale_shape_manual(values = c("Low" = 16, "High" = 17)) +
    labs(x = "STAI-Trait Score", y = "Interference (ms)") +
    theme_classic(base_size = 11) +
    theme(legend.position = "bottom")
}
...
if (sum(!is.na(lmm_data$IU)) > 2) {
  plot_list$IU <- ggplot(lmm_data %>% filter(!is.na(IU)),
                         aes(x = IU, y = interference, color = Group, shape = Prevalence)) +
    geom_point(size = 2, alpha = 0.6) +
    geom_smooth(method = "lm", se = TRUE, alpha = 0.2, linewidth = 1) +
    facet_wrap(~Prevalence, ncol = 2) +
    scale_color_manual(values = c("TD" = "#E64B35FF", "ASD" = "#4DBBD5FF")) +
    scale_shape_manual(values = c("Low" = 16, "High" = 17)) +
    labs(x = "IU Score", y = "Interference (ms)") +
    theme_classic(base_size = 11) +
    theme(legend.position = "bottom")
}
