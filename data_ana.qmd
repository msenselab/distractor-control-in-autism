...
subtitle: "Persistent belief updating despite perfect predictability – the atypical iterative prior updating account of autism"
author: "TO BE UPDATED AFTER DOUBLE BLIND PEER REVIEW"
date: "`r Sys.Date()`"
format:
  html:
    toc: true
...
The mixed ANOVA revealed a significant main effect of Group, and singleton, but not Prevalence. There was a significant interaction between Group and singleton. The other interactions were non-significant.
...
#| fig-cap: "Combined first fixation analysis. Top row: percentage of first fixations to target and distractor locations. There are group differences in attentional deployment patterns."
#| fig-width: 12
#| fig-height: 8
...
# Repeated measures ANOVA for first fixation percentages using aov_ez
...
create_anova_table(fixation_aov, "Repeated measures ANOVA for first fixation analysis (Singleton Distractor)")
...
# Function to fit nested linear models for each trait
fit_trait_models <- function(data, trait_var) {
  ...
  # Fit 3 nested models (simple linear regression - random effects removed
  # due to singular fit with only 2 observations per subject)
  model1 <- lm(reformulate(c(trait_var), "interference"), 
               data = valid_data)
  model2 <- lm(reformulate(c(trait_var, "Group_contrast", "Prevalence_contrast"), "interference"),
               data = valid_data)
  model3 <- lm(reformulate(c(paste0(trait_var, " * Group_contrast"), "Prevalence_contrast"), "interference"),
               data = valid_data)
...
bdi_models <- fit_trait_models(lm_data, "BDI_scaled")
stai_models <- fit_trait_models(lm_data, "STAI_Trait_scaled")
iu_models <- fit_trait_models(lm_data, "IU_scaled")
```
...
```{r lm-summary-table}
#| echo: false
#| tbl-cap: "Linear model results: Trait effects on distractor interference"

extract_lm_stats <- function(model, trait_name, model_name, trait_var) {
  ...
  if(length(group_idx) > 0) {
    ...
    group_p <- coefs[group_idx[1], "Pr(>|t|)"]
  } else {
    group_beta <- group_p <- NA
  }
  ...
  if(length(inter_idx) > 0) {
    ...
    inter_p <- coefs[inter_idx[1], "Pr(>|t|)"]
  } else {
    inter_beta <- inter_p <- NA
  }
  ...
  data.frame(
    ...
    Intercept = intercept_beta, Intercept_p = intercept_p,
    Trait_Beta = trait_beta, Trait_p = trait_p,
    Group_Beta = group_beta, Group_p = group_p,
    Prev_Beta = prev_beta, Prev_p = prev_p,
    Inter_Beta = inter_beta, Inter_p = inter_p,
...
if (!is.null(bdi_models)) {
  lm_summary <- rbind(lm_summary,
    extract_lm_stats(bdi_models$model1, "BDI", "1: Trait", "BDI_scaled"),
    extract_lm_stats(bdi_models$model2, "BDI", "2: +Group+Prev", "BDI_scaled"),
    extract_lm_stats(bdi_models$model3, "BDI", "3: +Trait×Group", "BDI_scaled")
  )
}

if (!is.null(stai_models)) {
  lm_summary <- rbind(lm_summary,
    extract_lm_stats(stai_models$model1, "STAI-Trait", "1: Trait", "STAI_Trait_scaled"),
    extract_lm_stats(stai_models$model2, "STAI-Trait", "2: +Group+Prev", "STAI_Trait_scaled"),
    extract_lm_stats(stai_models$model3, "STAI-Trait", "3: +Trait×Group", "STAI_Trait_scaled")
  )
}

if (!is.null(iu_models)) {
  lm_summary <- rbind(lm_summary,
    extract_lm_stats(iu_models$model1, "IU", "1: Trait", "IU_scaled"),
    extract_lm_stats(iu_models$model2, "IU", "2: +Group+Prev", "IU_scaled"),
    extract_lm_stats(iu_models$model3, "IU", "3: +Trait×Group", "IU_scaled")
  )
}
...
lm_summary %>%
  mutate(across(c(AIC, BIC), ~round(.x, 1)),
         ...
         across(c(Resid_Var, Resid_SD), ~round(.x, 2)),
  ...
  select(Trait, Model, AIC, BIC, 
         ...
         Trait_Beta, Trait_p, Trait_Sig,
         Group_Beta, Group_p, Group_Sig,
         Prev_Beta, Prev_p, Prev_Sig,
         Inter_Beta, Inter_p, Inter_Sig,
         Resid_Var, Resid_SD) %>%
  kable(col.names = c("Trait", "Model", "AIC", "BIC", 
                     ...
                     "β_Inter", "p", "",
                     "σ²_Resid", "σ_Resid"),
        caption = "Linear models: Coefficient table with beta values and residual variance. 
        ...
        align = c('l', 'l', rep('r', 18))) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), 
                font_size = 9,
                full_width = TRUE) %>%
  collapse_rows(columns = 1, valign = "top") %>%
  add_header_above(c(" " = 4, "Intercept" = 2, "Trait Effect" = 3, 
                     "Group Effect" = 3, "Prevalence Effect" = 3, 
                     "Interaction" = 3, "Residual" = 2))

# save the table as csv
#write_csv(lm_summary, file.path("C:/Users/whate/Downloads", "lm_trait_interference_summary.csv"))
```

### Model Equations

The coefficients in the table above can be used to construct prediction equations for each model. Note: These are simple linear regression models (not mixed-effects models) because there are only 2 observations per participant (one per prevalence condition), which is insufficient for reliable random effect estimation.

**Model 1: Trait-only model**
$$
\text{Interference}_i = \beta_0 + \beta_{\text{Trait}} \times \text{Trait}_i + \epsilon_i
$$

**Model 2: Main effects model**
$$
\text{Interference}_i = \beta_0 + \beta_{\text{Trait}} \times \text{Trait}_i + \beta_{\text{Group}} \times \text{Group}_i + \beta_{\text{Prev}} \times \text{Prev}_i + \epsilon_i
$$

**Model 3: Interaction model**
$$
\text{Interference}_i = \beta_0 + \beta_{\text{Trait}} \times \text{Trait}_i + \beta_{\text{Group}} \times \text{Group}_i + \beta_{\text{Prev}} \times \text{Prev}_i + \beta_{\text{Inter}} \times \text{Trait}_i \times \text{Group}_i + \epsilon_i
$$

Where:
- $i$ indexes observations
- $\beta_0$ = Intercept (mean interference when all predictors = 0)
...
- $\epsilon_i \sim N(0, \sigma^2_{\text{Resid}})$ = Residual error
...
$$
\text{Predicted Interference} = \beta_0 + (1 \times \beta_{\text{BDI}}) + (-1 \times \beta_{\text{Group}}) + (-1 \times \beta_{\text{Prev}})
$$

The actual observed value will also include the residual term $\epsilon_i$, which follows a normal distribution with variance $\sigma^2_{\text{Resid}}$.
...
if (!is.null(bdi_models)) {
  model2_coefs <- coef(bdi_models$model2)
  ...
  cat("Predicted interference =", round(pred, 2), "ms \n\n")
  ...
  pred2 <- model2_coefs["(Intercept)"] + 
           ...
           1 * model2_coefs["Group_contrast"] + 
           1 * model2_coefs["Prevalence_contrast"]
  cat("Predicted interference =", round(pred2, 2), "ms \n")
}
```
...
if (sum(!is.na(lm_data$BDI)) > 2) {
  ...
  plot_list$BDI <- ggplot() +
    ...
    geom_point(data = lm_data %>% filter(!is.na(BDI), Group == "ASD", Prevalence == "Low"),
               aes(x = BDI, y = interference), color = dark2_colors[1], 
               shape = 21, fill = NA, size = 2.5, alpha = 0.3, stroke = 1) +
    geom_smooth(data = lm_data %>% filter(!is.na(BDI), Group == "ASD", Prevalence == "Low"),
    ...
    scale_x_continuous(breaks = seq(0, max(lm_data$BDI, na.rm = TRUE), by = 10),
                       limits = c(min(lm_data$BDI, na.rm = TRUE), max(lm_data$BDI, na.rm = TRUE)),
...
# STAI-Trait
if (sum(!is.na(lm_data$STAI_Trait)) > 2) {
  # Create dummy data for legend
  legend_data <- data.frame(
    Group_Prev = factor(c("ASD/Low", "ASD/High", "TD/Low", "TD/High"),
                        levels = c("ASD/Low", "ASD/High", "TD/Low", "TD/High")),
  ...
  plot_list$STAI <- ggplot() +
    # ASD/Low Prevalence: color 1, dashed line, open circles
    geom_point(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "ASD", Prevalence == "Low"),
               aes(x = STAI_Trait, y = interference), color = dark2_colors[1], 
               shape = 21, fill = NA, size = 2.5, alpha = 0.3, stroke = 1) +
    geom_smooth(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "ASD", Prevalence == "Low"),
                aes(x = STAI_Trait, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[1], linetype = "dashed", alpha = 0.2, linewidth = 1) +
    # ASD/High Prevalence: color 2, dashed line, filled circles
    geom_point(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "ASD", Prevalence == "High"),
               aes(x = STAI_Trait, y = interference), color = dark2_colors[2], 
               shape = 16, size = 2.5, alpha = 0.3) +
    geom_smooth(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "ASD", Prevalence == "High"),
                aes(x = STAI_Trait, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[2], linetype = "dashed", alpha = 0.2, linewidth = 1) +
    # TD/Low Prevalence: color 3, solid line, open squares
    geom_point(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "TD", Prevalence == "Low"),
               aes(x = STAI_Trait, y = interference), color = dark2_colors[3], 
               shape = 22, fill = NA, size = 2.5, alpha = 0.3, stroke = 1) +
    geom_smooth(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "TD", Prevalence == "Low"),
                aes(x = STAI_Trait, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[3], linetype = "solid", alpha = 0.2, linewidth = 1) +
    # TD/High Prevalence: color 4, solid line, filled squares
    geom_point(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "TD", Prevalence == "High"),
               aes(x = STAI_Trait, y = interference), color = dark2_colors[4], 
               shape = 15, size = 2.5, alpha = 0.3) +
    geom_smooth(data = lm_data %>% filter(!is.na(STAI_Trait), Group == "TD", Prevalence == "High"),
                aes(x = STAI_Trait, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[4], linetype = "solid", alpha = 0.2, linewidth = 1) +
    # Add invisible points for legend
    geom_point(data = legend_data, aes(x = x, y = y, color = Group_Prev, shape = Group_Prev, linetype = Group_Prev),
    ...
    scale_x_continuous(breaks = seq(10, max(lm_data$STAI_Trait, na.rm = TRUE), by = 10),
                       limits = c(10, max(lm_data$STAI_Trait, na.rm = TRUE)))
}

# IU
if (sum(!is.na(lm_data$IU)) > 2) {
  # Create dummy data for legend
  legend_data <- data.frame(
    Group_Prev = factor(c("ASD/Low", "ASD/High", "TD/Low", "TD/High"),
                        levels = c("ASD/Low", "ASD/High", "TD/Low", "TD/High")),
  ...
  plot_list$IU <- ggplot() +
    # ASD/Low Prevalence: color 1, dashed line, open circles
    geom_point(data = lm_data %>% filter(!is.na(IU), Group == "ASD", Prevalence == "Low"),
               aes(x = IU, y = interference), color = dark2_colors[1], 
               shape = 21, fill = NA, size = 2.5, alpha = 0.3, stroke = 1) +
    geom_smooth(data = lm_data %>% filter(!is.na(IU), Group == "ASD", Prevalence == "Low"),
                aes(x = IU, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[1], linetype = "dashed", alpha = 0.2, linewidth = 1) +
    # ASD/High Prevalence: color 2, dashed line, filled circles
    geom_point(data = lm_data %>% filter(!is.na(IU), Group == "ASD", Prevalence == "High"),
               aes(x = IU, y = interference), color = dark2_colors[2], 
               shape = 16, size = 2.5, alpha = 0.3) +
    geom_smooth(data = lm_data %>% filter(!is.na(IU), Group == "ASD", Prevalence == "High"),
                aes(x = IU, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[2], linetype = "dashed", alpha = 0.2, linewidth = 1) +
    # TD/Low Prevalence: color 3, solid line, open squares
    geom_point(data = lm_data %>% filter(!is.na(IU), Group == "TD", Prevalence == "Low"),
               aes(x = IU, y = interference), color = dark2_colors[3], 
               shape = 22, fill = NA, size = 2.5, alpha = 0.3, stroke = 1) +
    geom_smooth(data = lm_data %>% filter(!is.na(IU), Group == "TD", Prevalence == "Low"),
                aes(x = IU, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[3], linetype = "solid", alpha = 0.2, linewidth = 1) +
    # TD/High Prevalence: color 4, solid line, filled squares
    geom_point(data = lm_data %>% filter(!is.na(IU), Group == "TD", Prevalence == "High"),
               aes(x = IU, y = interference), color = dark2_colors[4], 
               shape = 15, size = 2.5, alpha = 0.3) +
    geom_smooth(data = lm_data %>% filter(!is.na(IU), Group == "TD", Prevalence == "High"),
                aes(x = IU, y = interference), method = "lm", se = TRUE,
                color = dark2_colors[4], linetype = "solid", alpha = 0.2, linewidth = 1) +
    # Add invisible points for legend
    geom_point(data = legend_data, aes(x = x, y = y, color = Group_Prev, shape = Group_Prev, linetype = Group_Prev),
    ...
    scale_x_continuous(breaks = seq(20, 90, by = 10),
                       limits = c(20, max(lm_data$IU, na.rm = TRUE)))
}
...
if (length(plot_list) > 0) {
  ...
  # ggsave(file.path(output_dir, 'fig_trait_correlations_lm.png'),
  #        width = 10, height = 10, dpi = 300)
}
# combine all three trait plots into one figure with shared legend
ggsave(file.path(output_dir, 'fig_trait_correlations_combined.png'),
          plot = wrap_plots(plot_list, ncol = 1) + plot_layout(guides = "collect") &
            theme(legend.position = "bottom"), width = 10, height = 12, dpi = 300)
