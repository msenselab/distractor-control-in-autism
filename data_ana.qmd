...
Participants performed an additional singleton search task with the following manipulations: - **Distractor prevalence**: Low (50%) vs. High (90%) distractor frequency - **Singleton presence**: Distractor present vs. absent trials - **Precueing**: 100% valid spatial cues indicating potential distractor locations
...
```{r rt-visualization}
#| fig-cap: "Mean reaction times across experimental conditions. Error bars represent standard errors. The singleton effect (slower RTs when distractor present) shows significant group differences."
#| fig-width: 10
#| fig-height: 6

# Create bar plot with Prevalence as x-axis, color for Singleton, separate by Group
rt_plot <- mRTs %>%
  group_by(Group, Prevalence, Singleton) %>%
  summarise(
    mean_RT = mean(RT),
  ...
  ggplot(aes(x = Prevalence, y = mean_RT, fill = Singleton)) +
  ...
  labs(
    x = "Distractor Prevalence",
    y = "Reaction Time (ms)",
    fill = "Singleton Distractor",
    title = "Reaction Time by Prevalence and Singleton Condition"
  ) +
...
# Save the plot
ggsave(file.path(output_dir, 'fig_mRT.png'), rt_plot,
...
interference_plot <- mRTs_wide %>%
  ...
  ggplot(aes(x = Prevalence, y = mean_interference, fill = Group)) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7, alpha = 0.8) +
  geom_errorbar(aes(ymin = mean_interference - se_interference,
                    ymax = mean_interference + se_interference),
                position = position_dodge(width = 0.8), width = 0.2) +
  scale_fill_manual(values = c("TD" = "#1f77b4", "ASD" = "#ff7f0e")) +
  labs(
    x = "Distractor Prevalence",
    y = "Interference Effect (ms)",
    fill = "Group",
    title = "Distractor Interference by Group and Prevalence"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "top",
...
ggsave(file.path(output_dir, 'fig_mInterference.png'), interference_plot,
